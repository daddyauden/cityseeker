...
class MessageActions extends React.Component {
    ...
    async componentDidMount() {
        ...
        if (room.autoTranslate && message.u && message.u._id !== user.id) {
            this.options.push(
                I18n.t(message.autoTranslate ? 'View_Original' : 'Translate'),
            );
    ...
    allowEdit = props => {
        ...
        const {
            Message_AllowEditing: isEditAllowed,
    ...
    allowDelete = props => {
        ...
        const {
            Message_AllowDeleting: isDeleteAllowed,
    ...
    handlePermalink = async () => {
        ...
        EventEmitter.emit(LISTENER, {
            message: I18n.t('Permalink_copied_to_clipboard'),
        });
    };
    ...
    handleToggleTranslation = async () => {
        ...
        try {
            ...
            const translatedMessage = getMessageTranslation(
                message,
                room.autoTranslateLanguage,
            );
            if (!translatedMessage) {
                ...
                await RocketChat.translateMessage(
                    m,
                    room.autoTranslateLanguage,
                );
            }
