...
export default class Markdown extends PureComponent {
    ...
    renderText = ({context, literal}) => {
        ...
        return (
            <Text
                style={[styles.text, !preview ? defaultStyle : {}, ...style]}
    ...
    renderHashtag = ({hashtag}) => {
        const {channels, navToRoomInfo, style, preview} = this.props;
        return (
            <MarkdownHashtag
                hashtag={hashtag}
                channels={channels}
    ...
    renderAtMention = ({mentionName}) => {
        const {username, mentions, navToRoomInfo, preview, style} = this.props;
        return (
            <MarkdownAtMention
                mentions={mentions}
                mention={mentionName}
        ...
        return (
            <MarkdownEmoji
                ...
                isMessageContainsOnlyEmoji={
                    this.isMessageContainsOnlyEmoji && !preview
                }
    ...
    renderImage = ({src}) => (
        <Image style={styles.inlineImage} source={{uri: src}} />
    );

    renderEditedIndicator = () => (
        <Text style={styles.edited}> ({I18n.t('edited')})</Text>
    );
    ...
    render() {
        ...
        this.isMessageContainsOnlyEmoji =
            isOnlyEmoji(encodedEmojis) && emojiCount(encodedEmojis) <= 3;
