...
export default class MessageContainer extends React.Component {
    ...
    get isHeader() {
        ...
        if (
            this.hasError ||
            (previousItem && previousItem.status === messagesStatus.ERROR)
        ) {
            return true;
        }
        try {
    ...
    get isThreadSequential() {
        ...
        if (
            previousItem &&
            item.tmid &&
            (previousItem.tmid === item.tmid || previousItem.id === item.tmid)
        ) {
            return true;
        }
        return false;
    ...
    get isTemp() {
        const {item} = this.props;
        return (
            item.status === messagesStatus.TEMP ||
            item.status === messagesStatus.ERROR
        );
    }
    ...
    render() {
        ...
        if (autoTranslateRoom && autoTranslateMessage) {
            message =
                getMessageTranslation(item, autoTranslateLanguage) || message;
        }
