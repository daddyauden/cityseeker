...
class LoginSignupView extends React.Component {
    ...
    onPressGitlab = () => {
        ...
        const baseURL = Gitlab_URL
            ? Gitlab_URL.trim().replace(/\/*$/, '')
            : 'https://gitlab.com';
    ...
    onPressCustomOAuth = loginService => {
        ...
        const url = absolutePath.includes(domain)
            ? absolutePath
            : domain + absolutePath;
        this.openOAuth({url});
    };
        ...
        if (length > 3) {
            return (
                <View style={styles.servicesTogglerContainer}>
                    <View
                        style={[styles.separatorLine, styles.separatorLineLeft]}
                    />
                    <BorderlessButton onPress={this.toggleServices}>
                        <Image
                            source={{uri: 'options'}}
                            style={[
                                styles.servicesToggler,
                                !collapsed && styles.inverted,
                            ]}
                        />
                    </BorderlessButton>
                    <View
    ...
    renderItem = service => {
        ...
        let buttonText;
        if (
            service.service === 'saml' ||
            (service.service === 'cas' && CAS_enabled)
        ) {
