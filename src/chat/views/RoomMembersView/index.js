...
class RoomMembersView extends React.Component {
    ...
    onSearchChangeText = protectedFunction(text => {
        ...
        if (members && members.length > 0 && text) {
            membersFiltered = members.filter(m =>
    ...
    onPressUser = async item => {
        try {
            ...
            const query = await subsCollection
                .query(Q.where('name', item.username))
                .fetch();
            if (query) {
                const [room] = query;
                this.goRoom({rid: room.rid, name: item.username, room});
            } else {
                const result = await RocketChat.createDirectMessage(
                    item.username,
                );
                if (result.success) {
    ...
    toggleStatus = () => {
        try {
            ...
            this.setState(
                {members: [], allUsers: !allUsers, end: false},
                () => {
                    this.fetchMembers();
                },
            );
    ...
    handleMute = async () => {
        ...
        try {
            ...
            EventEmitter.emit(LISTENER, {
                message: I18n.t('User_has_been_key', {
