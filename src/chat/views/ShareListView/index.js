...
class ShareListView extends React.Component {
    static navigationOptions = ({navigation}) => {
        ...
        return {
            ...
            headerTitle: (
                <ShareListHeader searching={searching} search={search} />
            ),
    ...
    getSubscriptions = async (server, fileInfo) => {
        ...
        if (server) {
            ...
            const canUploadFileResult = canUploadFile(
                fileInfo || fileData,
                serverInfo,
            );
    ...
    uriToPath = uri =>
        decodeURIComponent(isIOS ? uri.replace(/^file:\/\//, '') : uri);
    ...
    renderItem = ({item}) => {
        ...
        return (
            <DirectoryItem
                ...
                description={
                    item.t === 'c' ? item.topic || item.description : item.fname
                }
                type={item.t}
    ...
    renderSelectServer = () => {
        ...
        const currentServer = servers.find(
            serverFiltered => serverFiltered.id === server,
        );
    ...
    renderContent = () => {
        ...
        return (
            <FlatList
                ...
                ListEmptyComponent={
                    searching && searchText ? this.renderEmptyComponent : null
                }
                enableEmptySections
    ...
    renderError = () => {
        const {fileInfo: file, loading, searching, error} = this.state;
        ...
        return (
            <View style={styles.container}>
                {!searching ? <>{this.renderSelectServer()}</> : null}
