...
class RightButtonsContainer extends React.PureComponent {
    ...
    async componentDidMount() {
        ...
        if (tmid) {
            ...
            try {
                const threadRecord = await db.collections
                    .get('messages')
                    .find(tmid);
    ...
    updateThread = thread => {
        ...
        this.setState({
            isFollowingThread:
                thread.replies && !!thread.replies.find(t => t === userId),
        });
    };
    ...
    goRoomActionsView = () => {
        const {rid, t, navigation, room} = this.props;
    ...
    render() {
        ...
        return (
            <MaterialCommunityIcons
                ...
                style={[Style.f_size_17, Style.f_color_3, Style.m_r_3]}
            />
        );
    }
}
