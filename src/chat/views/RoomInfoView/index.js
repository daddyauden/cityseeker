...
const getRoomTitle = (room, type, name) =>
    type === 'd' ? (
            ...
            headerRight: showEdit ? (
                <CustomHeaderButtons>
                    <Item
                        iconName="edit"
                        onPress={() =>
                            navigation.navigate('RoomInfoEditView', {rid})
                        }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (roles && roles.length) {
                        parsedRoles = await Promise.all(
                            roles.map(async role => {
                                const description = await this.getRoleDescription(
                                    role,
                                );
                                return description;
                            }),
    ...
    renderItem = (key, room) => (
        <View style={styles.item}>
            ...
            <Text
                style={[
                    styles.itemContent,
                    !room[key] && styles.itemContent__empty,
                ]}
