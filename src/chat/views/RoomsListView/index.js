...
import {CityBtn, BottomTabBar} from '../../../common/views/components';
...
class RoomsListView extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        const {loading, searching, width, search} = this.state;
    ...
    getSubscriptions = debounce(
        async () => {
            ...
            const {sortBy, showUnread, showFavorites, groupByType} = this.props;
            ...
            this.querySubscription = observable.subscribe(data => {
                ...
                if (showUnread) {
                    unread = chats.filter(
                        s => (s.unread > 0 || s.alert) && !s.hideUnreadStatus,
                    );
                } else {
                    unread = [];
                }
                ...
                if (groupByType) {
                    ...
                    chats = chats.filter(
                        s => (!s.unread && !s.alert) || s.hideUnreadStatus,
                    );
                }
    ...
    renderItem = ({item}) => {
        ...
        const {userId, username, token, baseUrl, StoreLastMessage} = this.props;
    ...
    renderSectionHeader = header => (
        <View
            style={
                ...
                [Style.p_h_3, Style.p_t_4, Style.p_b_2, Style.bg_color_15]
            }>
            <Text
                style={
                    // styles.groupTitle
        ...
        if (!(showUnread || showFavorites || groupByType)) {
            ...
            return (
                <FlatList
                    ...
                    keyExtractor={keyExtractor}
                    style={[Style.w_100, Style.bg_color_15]}
                    renderItem={this.renderItem}
