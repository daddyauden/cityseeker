...
const formatMsg = ({lastMessage, type, showLastMessage, username}) => {
    if (!showLastMessage) {
        return '';
    }
    ...
    if (
        ...
        Object.keys(lastMessage.attachments).length
    ) {
        const user = isLastMessageSentByMe
            ? I18n.t('You')
            : lastMessage.u.username;

        const {audio_type, image_type, video_type} = lastMessage.attachments[0];

        let attachmentType = 'attachment';

        if (audio_type) {
            attachmentType = I18n.t('audio');
        } else if (image_type) {
            attachmentType = I18n.t('image');
        } else if (video_type) {
            attachmentType = I18n.t('video');
        }

        return I18n.t('User_sent_an_attachment', {user, attachmentType});
    }
