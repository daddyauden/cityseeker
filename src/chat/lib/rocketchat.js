...
import {shareSelectServer, shareSetUser} from '../actions/share';
...
const TOKEN_KEY = 'WZ82tWZ82t6Q8p4p4';
const SORT_PREFS_KEY = 'RC_WZ82tWZ_2tWZ82t6Q8_tWZ';
export const MARKDOWN_KEY = 'WZ82tWZ_MA8p4pWN_p4p4';
export const CRASH_REPORT_KEY = '2tWZ_2tWZ_Z82_82t';
...
import RocketChatExtra from '../../common/lib/rocketchat';
...
const RocketChat = {
    ...
    async getWebsocketInfo({server}) {
        ...
        const sdk = new RocketchatClient({
            host: server,
            protocol: 'ddp',
            useSsl,
        });
    ...
    async getServerInfo(server) {
        ...
        try {
            const result = await fetch(`${server}/api/info`).then(
                async response => {
                    let res = notRCServer;
                    try {
        ...
        return new Promise(resolve => {
            ...
            this.usersListener = this.sdk.onStreamData(
                ...
                protectedFunction(ddpMessage =>
                    RocketChat._setUser(ddpMessage),
                ),
            );
                ...
                protectedFunction(ddpMessage => {
                    ...
                    if (eventName === 'user-status') {
                        ...
                        if (!this._setUserTimer) {
                            this._setUserTimer = setTimeout(() => {
                                ...
                                InteractionManager.runAfterInteractions(() => {
                                    reduxStore.dispatch(
                                        setActiveUsers(activeUsersBatch),
                                    );
                                });
                        ...
                        if (loggedUser && loggedUser.id === id) {
                            reduxStore.dispatch(
                                setUser({status: STATUSES[status]}),
                            );
                        }
                    }
                }),
    ...
    async shareExtensionInit(server) {
        ...
        // set User info
        try {
            const userId = await RNUserDefaults.get(
    ...
    async login(params) {
        try {
            ...
            if (
                e.data &&
                e.data.message &&
                /you've been logged out by the server/i.test(e.data.message)
            ) {
    ...
    async logout({server}) {
        ...
        try {
            ...
            await RNUserDefaults.setObjectForKey(
                SERVERS,
                servers && servers.filter(srv => srv[SERVER_URL] !== server),
            );
            // clear certificate for server - SSL Pinning
            const certificate = await RNUserDefaults.objectForKey(
                extractHostname(server),
            );
    ...
    registerPushToken() {
        return new Promise(async resolve => {
            ...
            console.info('registerPushToken', token);
            if (token) {
    ...
    removePushToken() {
        const token = getDeviceToken();
        if (token) {
            console.info('removePushToken', token);
            // RC 0.60.0
    ...
    async search({text, filterUsers = true, filterRooms = true}) {
        ...
        if (filterUsers && filterRooms) {
            data = data.filter(
                item => item.t === 'd' || item.t === 'c' || item.t === 'p',
            );
        } else if (filterUsers && !filterRooms) {
            data = data.filter(item => item.t === 'd');
        } else if (!filterUsers && filterRooms) {
            data = data.filter(item => item.t === 'c');
        }
        data = data.slice(0, 7);
        ...
        try {
            if (data.length < 7) {
                const {users, rooms} = await Promise.race([
                    ...
                    new Promise(
                        (resolve, reject) => (this.oldPromise = reject),
                    ),
                ]);
    ...
    editMessage(message) {
        ...
        // RC 0.49.0
        return this.sdk.post('chat.update', {
            roomId: rid,
            msgId: id,
            text: msg,
        });
    ...
    reportMessage(messageId) {
        return this.sdk.post('chat.reportMessage', {
            messageId,
            description: 'Message reported by user',
        });
    },
    async getRoom(rid) {
        try {
    ...
    toggleArchiveRoom(roomId, t, archive) {
        if (archive) {
            // RC 0.48.0
            return this.sdk.post(`${this.roomTypeToApiType(t)}.archive`, {
                roomId,
            });
        }
        // RC 0.48.0
        return this.sdk.post(`${this.roomTypeToApiType(t)}.unarchive`, {
            roomId,
        });
    },
    hideRoom(roomId, t) {
    ...
    async hasPermission(permissions, rid) {
        ...
        try {
            ...
            return permissions.reduce((result, permission) => {
                ...
                if (permissionFound) {
                    result[permission] = returnAnArray(
                        permissionFound.roles,
                    ).some(r => mergedRoles.includes(r));
                }
                return result;
            }, {});
        } catch (e) {
    ...
    async getLoginServices(server) {
        try {
            let loginServices = [];
            const loginServicesResult = await fetch(
                `${server}/api/v1/settings.oauth`,
            ).then(response => response.json());

            if (
                loginServicesResult.success &&
                loginServicesResult.services.length > 0
            ) {
                const {services} = loginServicesResult;
                loginServices = services;

                const loginServicesReducer = loginServices.reduce(
                    (ret, item) => {
                        const name =
                            item.name || item.buttonLabelText || item.service;
    ...
    getUserPresence() {
        return new Promise(async resolve => {
            ...
            if (
                serverVersion &&
                semver.lt(semver.coerce(serverVersion), '1.1.0')
            ) {
                if (this.activeUsersSubTimeout) {
                    clearTimeout(this.activeUsersSubTimeout);
                    this.activeUsersSubTimeout = false;
    ...
    async canAutoTranslate() {
        const db = database.active;
        try {
            const AutoTranslate_Enabled =
                reduxStore.getState().settings &&
                reduxStore.getState().settings.AutoTranslate_Enabled;
            if (!AutoTranslate_Enabled) {
                return false;
            }
            const permissionsCollection = db.collections.get('permissions');
            const autoTranslatePermission = await permissionsCollection.find(
                'auto-translate',
            );
            const userRoles =
                (reduxStore.getState().login.user &&
                    reduxStore.getState().login.user.roles) ||
                [];
            return autoTranslatePermission.roles.some(role =>
