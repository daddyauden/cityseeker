...
const serverInfoUpdate = async (serverInfo, iconSetting) => {
    ...
    let info = serverInfo.reduce((allSettings, setting) => {
        ...
        if (setting._id === 'FileUpload_MediaTypeWhiteList') {
            return {
                ...allSettings,
                FileUpload_MediaTypeWhiteList: setting.valueAsString,
            };
        }
        if (setting._id === 'FileUpload_MaxFileSize') {
            return {
                ...allSettings,
                FileUpload_MaxFileSize: setting.valueAsNumber,
            };
        }
        return allSettings;
    }, {});
    ...
    if (iconSetting) {
        const iconURL = `${serverId}/${iconSetting.value.url ||
...
export async function setSettings() {
    ...
    reduxStore.dispatch(
        actions.setAllSettings(
            RocketChat.parseSettings(parsed.slice(0, parsed.length)),
        ),
    );
}
...
export default async function() {
    try {
        ...
        const result = await fetch(
            `${this.sdk.client.host}/api/v1/settings.public?query={"_id":{"$in":${settingsParams}}}`,
        ...
        const filteredSettings = this._prepareSettings(
            data.filter(item => item._id !== 'Assets_favicon_512'),
        );
        ...
        InteractionManager.runAfterInteractions(async () => {
            // filter server info
            const serverInfo = filteredSettings.filter(i1 =>
                serverInfoKeys.includes(i1._id),
            );
            const iconSetting = data.find(
                item => item._id === 'Assets_favicon_512',
            );
            ...
            await db.action(async () => {
                ...
                // filter settings
                let settingsToCreate = filteredSettings.filter(
                    i1 => !allSettingsRecords.find(i2 => i1._id === i2.id),
                );
                let settingsToUpdate = allSettingsRecords.filter(i1 =>
                    filteredSettings.find(i2 => i1.id === i2._id),
                );
                ...
                settingsToUpdate = settingsToUpdate.map(setting => {
                    const newSetting = filteredSettings.find(
                        s => s._id === setting.id,
                    );
