...
const updatePermissions = async ({update = [], remove = [], allRecords}) => {
    ...
    if (update && update.length) {
        permissionsToCreate = update.filter(
            i1 => !allRecords.find(i2 => i1._id === i2.id),
        );
        permissionsToUpdate = allRecords.filter(i1 =>
    ...
    if (remove && remove.length) {
        permissionsToDelete = allRecords.filter(i1 =>
            remove.find(i2 => i1.id === i2._id),
        );
        permissionsToDelete = permissionsToDelete.map(permission =>
            permission.prepareDestroyPermanently(),
        );
    }
