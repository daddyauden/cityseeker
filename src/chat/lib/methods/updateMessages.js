...
export default function updateMessages({rid, update = [], remove = []}) {
    try {
        ...
        return db.action(async () => {
            ...
            const threadMessagesCollection = db.collections.get(
                'thread_messages',
            );
            ...
            // filter messages
            let msgsToCreate = update.filter(
                i1 => !allMessagesRecords.find(i2 => i1._id === i2.id),
            );
            let msgsToUpdate = allMessagesRecords.filter(i1 =>
                update.find(i2 => i1.id === i2._id),
            );

            // filter threads
            const allThreads = update.filter(m => m.tlm);
            let threadsToCreate = allThreads.filter(
                i1 => !allThreadsRecords.find(i2 => i1._id === i2.id),
            );
            let threadsToUpdate = allThreadsRecords.filter(i1 =>
                allThreads.find(i2 => i1.id === i2._id),
            );

            // filter thread messages
            const allThreadMessages = update.filter(m => m.tmid);
            let threadMessagesToCreate = allThreadMessages.filter(
            ...
            threadMessagesToCreate = threadMessagesToCreate.map(threadMessage =>
                threadMessagesCollection.prepareCreate(
                    protectedFunction(tm => {
                        tm._raw = sanitizedRaw(
                            {id: threadMessage._id},
                            threadMessagesCollection.schema,
                        );
            ...
            threadMessagesToUpdate = threadMessagesToUpdate.map(
                threadMessage => {
                    const newThreadMessage = allThreadMessages.find(
                        t => t._id === threadMessage.id,
                    );
            ...
            if (remove && remove.length) {
                msgsToDelete = allMessagesRecords.filter(i1 =>
                    remove.find(i2 => i1.id === i2._id),
                );
                msgsToDelete = msgsToDelete.map(m =>
                    m.prepareDestroyPermanently(),
                );
                threadsToDelete = allThreadsRecords.filter(i1 =>
                    remove.find(i2 => i1.id === i2._id),
                );
                threadsToDelete = threadsToDelete.map(t =>
                    t.prepareDestroyPermanently(),
                );
                threadMessagesToDelete = allThreadMessagesRecords.filter(i1 =>
                    remove.find(i2 => i1.id === i2._id),
                );
                threadMessagesToDelete = threadMessagesToDelete.map(tm =>
                    tm.prepareDestroyPermanently(),
                );
            }
