                ...
                InteractionManager.runAfterInteractions(async () => {
                    try {
                        ...
                        let threadMessagesToCreate = data.filter(
                            i1 =>
                                !allThreadMessagesRecords.find(
                                    i2 => i1._id === i2.id,
                                ),
                        );
                        let threadMessagesToUpdate = allThreadMessagesRecords.filter(
                            i1 => data.find(i2 => i1.id === i2._id),
                        );

                        threadMessagesToCreate = threadMessagesToCreate.map(
                            threadMessage =>
                        ...
                        threadMessagesToUpdate = threadMessagesToUpdate.map(
                            threadMessage => {
                                const newThreadMessage = data.find(
