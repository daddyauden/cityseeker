            ...
            if (roles && roles.length) {
                InteractionManager.runAfterInteractions(async () => {
                    await db.action(async () => {
                        ...
                        const allRolesRecords = await rolesCollections
                            .query()
                            .fetch();

                        // filter roles
                        let rolesToCreate = roles.filter(
                            i1 => !allRolesRecords.find(i2 => i1._id === i2.id),
                        );
                        let rolesToUpdate = allRolesRecords.filter(i1 =>
                            roles.find(i2 => i1.id === i2._id),
                        );
