...
const updateEmojis = async ({update = [], remove = [], allRecords}) => {
    ...
    if (update && update.length) {
        emojisToCreate = update.filter(
            i1 => !allRecords.find(i2 => i1._id === i2.id),
        );
        emojisToUpdate = allRecords.filter(i1 =>
    ...
    if (remove && remove.length) {
        emojisToDelete = allRecords.filter(i1 =>
            remove.find(i2 => i1.id === i2._id),
        );
        emojisToDelete = emojisToDelete.map(emoji =>
            emoji.prepareDestroyPermanently(),
        );
    }
            ...
            if (serverVersion && semver.lt(serverVersion, '0.75.0')) {
                ...
                InteractionManager.runAfterInteractions(async () => {
                    ...
                    emojis = emojis.filter(
                        emoji =>
                            !updatedSince || emoji._updatedAt > updatedSince,
                    );
