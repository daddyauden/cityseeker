...
export default function subscribeRoom({rid}) {
    ...
    const handleMessageReceived = protectedFunction(ddpMessage => {
        const message = buildMessage(
            EJSON.fromJSONValue(ddpMessage.fields.args[0]),
        );
        ...
        InteractionManager.runAfterInteractions(async () => {
            ...
            if (message.tmid) {
                try {
                    threadMessageRecord = await threadMessagesCollection.find(
                        message._id,
                    );
                } catch (error) {
                    // Do nothing
    ...
    notifyRoomListener = this.sdk.onStreamData(
        'stream-notify-room',
