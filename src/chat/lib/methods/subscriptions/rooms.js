...
export default function subscribeRooms() {
        ...
        if (/subscriptions/.test(ev)) {
            if (type === 'removed') {
                try {
                    ...
                    const messagesToDelete = messages.map(m =>
                        m.prepareDestroyPermanently(),
                    );
                    const threadsToDelete = threads.map(m =>
                        m.prepareDestroyPermanently(),
                    );
                    const threadMessagesToDelete = threadMessages.map(m =>
                        m.prepareDestroyPermanently(),
                    );
                    await db.action(async () => {
                        await db.batch(
    ...
    streamListener = this.sdk.onStreamData(
        'stream-notify-user',
