import {put, take, takeLatest, fork, cancel, race} from 'redux-saga/effects';
    ...
    try {
        ...
        if (!serverInfo.success || !websocketInfo.success) {
            if (raiseError) {
                ...
                log(new Error(I18n.t(info.message, info.messageOptions)));
            }
...
const handleSelectServer = function* handleSelectServer({
    server,
    version,
    fetchVersion,
}) {
    try {
        ...
        const userId = yield RNUserDefaults.get(
        ...
        if (userId) {
            try {
                ...
                const userCredentials =
                    servers && servers.find(srv => srv[SERVER_URL] === server);
...
const handleServerRequest = function* handleServerRequest({
    ...
    try {
        if (certificate) {
            yield RNUserDefaults.setObjectForKey(
