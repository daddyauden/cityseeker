...
import {list, remove, addItemAlbum} from '../../actions/image';
import AutoImageWidth from '../../components/AutoImageWidth';
import {update_banner} from '../../actions/item';
...
class Default extends React.Component {
    static navigationOptions = ({navigation, screenProps}) => {
        const {trans} = screenProps.system;
    ...
    _changeBannerImage = () => {
        ...
        ImagePicker.openPicker(options)
            .then(image => {
                ...
                update_banner(data).then(response => {
                    const {status, message} = response;
    ...
    _postRequest = () => {
        const {item, album, uploadImages} = this.state;
    ...
    _renderLocalImages = () => {
        ...
        const {params, trans} = this.props.system;
    ...
    _renderBanner = () => {
        const {trans} = this.props.system;
        const {banner} = this.state.item;

        const data = (
            <TouchableWithoutFeedback onPress={() => this._changeBannerImage()}>
                <View style={[Style.column_center, Style.row_center]}>
                    {banner !== undefined &&
                    banner !== null &&
                    banner !== '' ? (
                        <AutoImageWidth
                            uri={Common.load_image(banner)}
                            p_width={WIDTH}
                            p_style={{
                                width: WIDTH,
